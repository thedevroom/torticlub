...
export function FrameSequence({ className }: { className?: string }) {
  ...
  const draw = useCallback((index: number) => {
    ...
    // Contain fit — never crop pieces; gaps stay proportional on any viewport
...
const labels = ["SOLO", "DUO", "CLUB"] as const;

  return (
    <section
      ref={sectionRef}
      className={cn("relative h-[380vh]", className)}
      aria-label="Secuencia SOLO, DUO y CLUB"
    >
      <div className="sticky top-0 h-[100svh] flex flex-col items-center justify-center bg-surface overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-[1400px] px-5 sm:px-6 md:px-10 lg:px-12 grid lg:grid-cols-12 gap-8 lg:gap-10 items-center h-full py-20 md:py-24">
          <div className="lg:col-span-4 order-2 lg:order-1">
            ...
            <p className="mt-5 text-ink/50 font-medium leading-relaxed max-w-sm">
              Entera. Dos mitades. Cuatro cuartos. Scroll para ver el ritual
              SOLO → DUO → CLUB.
            </p>
            <div className="mt-8 flex flex-wrap gap-2">
              {labels.map((text) => (
                <span
                  key={text}
                  className="rounded-full border border-ink/10 bg-[#FBF8F1] px-3 py-1.5 text-[0.65rem] font-bold tracking-[0.18em] text-ink/50"
                >
                  {text}
                </span>
              ))}
          ...
          <div className="lg:col-span-8 order-1 lg:order-2 relative min-h-0">
            {/*
              Generous square stage: pieces can separate without clipping.
              object-fit is handled in canvas (contain), not CSS crop.
            */}
            <div className="relative mx-auto w-full max-w-[min(94vw,680px)] aspect-square">
              {!ready && (
